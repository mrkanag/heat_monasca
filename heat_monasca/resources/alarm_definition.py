...
class MonascaAlarmDefinition(resource.Resource):
    ...
    properties_schema = {
        ...
        OK_ACTIONS: properties.Schema(
            properties.Schema.LIST,
            _('The notification method to use when an alarm state is OK.'),
            update_allowed=True,
        ...
        ALARM_ACTIONS: properties.Schema(
            properties.Schema.LIST,
            _('The notification method to use when an alarm state is ALARM.'),
            update_allowed=True,
            schema=properties.Schema(
                properties.Schema.STRING,
                _('Monasca notification'),
        ...
        UNDETERMINED_ACTIONS: properties.Schema(
            properties.Schema.LIST,
            _('The notification method to use when an alarm state is '
              'UNDETERMINED.'),
            update_allowed=True,
            schema=properties.Schema(
                properties.Schema.STRING,
    ...
    def handle_update(self, json_snippet=None, tmpl_diff=None, prop_diff=None):
        ...
        args['expression'] = (prop_diff.get(self.EXPRESSION) or
                              self.properties[self.EXPRESSION])
        ...
        args['actions_enabled'] = (prop_diff.get(self.ACTIONS_ENABLED) or
                                   self.properties[self.ACTIONS_ENABLED])

        args['match_by'] = self.properties.get(self.MATCH_BY)

        try:
            self.client().alarm_definitions.update(**args)
